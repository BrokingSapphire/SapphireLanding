...
const FeatureBox: React.FC<FeatureBoxProps> = ({ icon, title, description }) => {
  return (
    <div className="flex-1 h-[191px] bg-[#F5F7FA] border-t-[0.5px] border-gray-300 flex flex-col gap-2 opacity-100 px-7 py-6">
      ...
      {/* Description */}
      <p
        className="text-gray-600 opacity-100"
        style={{
          fontFamily: 'Poppins',
          fontWeight: 400,
          fontSize: '16px',
...
const ReasonsToChooseUs: React.FC<ReasonsToChooseUsProps> = ({ className = '' }) => {
  ...
  return (
    <div className={`w-full  h-[325px] bg-[#F5F7FA] mx-auto flex flex-col gap-6 opacity-100 ${className}`}
         style={{ padding: '48px 85px' }}>
      
      {/* Main Heading */}
      <div className="w-full h-[40px] opacity-100 flex items-center justify-center">
        <h2 
          className="text-black text-center opacity-100"
          style={{
            fontFamily: 'Lexend',
            fontWeight: 500,
            fontSize: '32px',
      ...
      <div className="w-full h-[191px] flex opacity-100">
